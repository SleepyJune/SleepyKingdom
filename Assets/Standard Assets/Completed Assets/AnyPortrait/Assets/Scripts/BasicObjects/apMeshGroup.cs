...
namespace AnyPortrait
{
	...
	public class apMeshGroup : MonoBehaviour
	{
		...
		public void ChangeRenderUnitDetph(apRenderUnit renderUnit, int nextDepth)
		{
			...
			//if(sameLevelRenderUnits.Count == 0)
			//{
			//	Debug.LogError("대상 RenderUnit이 없다.");
			//}
			//else
			//{
			//	Debug.Log("교체 대상이 되는 RenderUnits : " + sameLevelRenderUnits.Count);
			//	for (int i = 0; i < sameLevelRenderUnits.Count; i++)
			//	{
			//		Debug.Log("[Depth : " + sameLevelRenderUnits[i].GetDepth() + "] " + sameLevelRenderUnits[i].Name);
			//	}
			//}
			...
			if (renderUnit.GetDepth() < nextDepth) { isIncrease = true; }
			 ...
			 }))
			{
				...
				for (int i = 0; i < sameLevelRenderUnits.Count; i++)
				{
					...
					if (isIncrease)
					{
						//if (renderUnit._depth < nextUnit._depth)//이전
